...
addenvirLC2 <- function(envir, landcov) { ## start of main function
  ...
  envstack <- crop(envstack, LCStack) # crop the climate data to the extent of the land cover data (needed because the climate data has a global extent and the land cover data has an European extent)
  
  envstack2 <- stack(envstack, LCStack) # stack climate and land cover layers
  
  corr <- layerStats(envstack2, 'pearson', na.rm=T) # correlation test of environmental variables of choice
  ...
  # extract climate values for grid cells in which the target species occurs
  ...
  colnames(occenv) <- c(names(occenv[1:length(PAenv),]), landcov[1])} else{occenv <- cbind(occenv, raster::extract(x = LCStack, y = data.frame(occ[,c('decimalLongitude','decimalLatitude')])))}
  # extract land cover values for the grid cell in which the target species occurs
  
  occenv <- occenv[complete.cases(occenv),] # remove occurrence points without environmental data
  
  print("Note: The following land cover variables only take on the value 0 accross the occurrences of your focal species. Please remove/replace these variables to avoid errors while model fitting due to uninformative predictors.") # notification for the user
